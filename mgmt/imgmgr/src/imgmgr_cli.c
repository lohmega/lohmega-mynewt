...
imgr_cli_show_slot(int slot)
{
    ...
    uint8_t state_flags;
    int magic_version = 1;
    if (imgr_magic(slot) == IMAGE_MAGIC_V2) {
        magic_version = 2;
    }
    ...
    console_printf("%d %8s: %s %s (v%d)\n",
      slot, ver_str,
      hex_format(hash, IMGMGR_HASH_LEN, hash_str, sizeof(hash_str)),
      imgr_cli_flags_str(flags, state_flags), magic_version);
}
...
imgr_cli_cmd(int argc, char **argv)
{
    ...
    } else if (!strcmp(argv[1], "v2c")) {
        imgr_v2image_set_confirmed();
    } else if (!strcmp(argv[1], "im2bl")) {
        image_to_bl(2);
